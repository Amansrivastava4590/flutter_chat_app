...
class Whatsapp {
  ...
  static Future<meModel> Me()async{
    final res = await http.get(Uri.parse("$whatsappUrl/me"));
  ...
  static Future<List<ChatModel>> Chats()async{
    final res = await http.get(Uri.parse("$whatsappUrl/chats"));
  ...
  static Future<List<CallModel>> Call()async{
    final res = await http.get(Uri.parse("$whatsappUrl/calls"));
    //Response response = await get(Uri.parse("$whatsappUrl/chats"));
    if(res.statusCode == 200){
      final parsed = jsonDecode(res.body);
  ...
  static Future<List<PeopleModel>> People()async{
    try {
      final res = await http.get(Uri.parse("$whatsappUrl/people"),
